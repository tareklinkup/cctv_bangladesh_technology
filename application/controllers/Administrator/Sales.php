...
class Sales extends CI_Controller {
    ...
    public function allServiceRequest()
    {
        ...
        $data['title'] = "Service Request Record";

        $data['content'] = $this->load->view('Administrator/sales/all_service_request', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    
    }


    public function getProductServiceRecord()
    {
        ...
        if(isset($data->status) && $data->status != ''){
            $clauses .= " and pss.status = '$data->status'";
        }

        $getServicRecord = $this->db->query(" SELECT 
        pss.*, 
        p.Product_Name, 
        c.Customer_Code,
        c.Customer_Name,
        ps.ps_serial_number
         FROM `tbl_product_service` pss 
        LEFT JOIN tbl_product p on p.Product_SlNo = pss.product_id
        LEFT JOIN tbl_product_serial_numbers ps on ps.ps_serial_number = pss.ps_number
        left join tbl_customer c on c.Customer_SlNo = pss.Customer_id
        WHERE pss.ps_brunchId = '$data->branchId'
        and pss.status != 'd'
        $clauses
        ORDER BY pss.Customer_id
        ")->result();
    ...
    public function getTransfaredServiceList()
    {
        ...
        LEFT JOIN tbl_product_serial_numbers ps on ps.ps_serial_number = pss.ps_number
        left join tbl_customer c on c.Customer_SlNo = pss.Customer_id
        WHERE pss.service_branch_id = '$data->branchId'
        and pss.status = 'p'
        GROUP BY pss.Customer_id
        ")->result();
    ...
    public function serviceApprovedList()
    {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Service Approved List";

        $data['content'] = $this->load->view('Administrator/sales/approved_service_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    
    }

    public function transfaredService()
    {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Service Transfared List";

        $data['content'] = $this->load->view('Administrator/sales/transfared_service_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    
    }

    public function getProductServiceRecordUpdate()
    {
        $data = json_decode($this->input->raw_input_stream);
        ...
        ps.note,
        b.Brunch_name
         FROM `tbl_product_service` ps 
         left join tbl_brunch b on b.brunch_id = ps.ps_brunchId
        WHERE ps.status = 'p'
        and ps.product_service_id = ?
        ", $data->serviceId)->row();
        echo json_encode($getServicRecord);
    }


    public function updatePendingService()
    {
        
        $data = json_decode($this->input->raw_input_stream);

        $this->db->set('service_branch_id', $data->branchId)->where('product_service_id', $data->service_id)->update('tbl_product_service');
